...
;;; fix the bug in cnblogs-replace-media-object-location
(defun erase-the-file-prefix (str)
	  ...
	  (let ((regexp 
	    ...
	    (while (string-match regexp
	      ...
	      (let* ((media-path (match-string 0
		     ...
		     (media-url
		      (save-match-data
			(and (file-exists-p (erase-the-file-prefix media-path))
			     (cnblogs-metaweblog-new-media-object 
			      (cnblogs-make-media-object-file-data
			       (erase-the-file-prefix media-path)))))))
