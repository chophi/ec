...
(defcustom topcoder-template-python
  ""
  "TopCoder Python code template"
  :type '(string)
  :group 'topcoder)
    ...
    (cond ((eq language 'cpp)
	  ...
	  ((eq language 'vb)
	   (topcoder-parse-template topcoder-template-vb desc1))
          ((eq language 'python)
           (topcoder-parse-template topcoder-template-python desc1)))))
...
(defun topcoder-get-language ()
  (if (boundp 'topcoder-language)
	...
	(let ((ls (match-string 1 (buffer-name))))
	  (if (member ls '("cpp" "java" "cs" "vb" "python"))
	      (intern ls)
	    'cpp))
      'cpp)))
...
(defun topcoder-set-problem (desc)
  ...
  (let ((language (cdr (assoc 'language desc)))
    ...
    (when (fboundp 'my-perfect-side-window)
      (my-perfect-side-window))
...
(defun topcoder-compile-command ()
  (let ((language (topcoder-get-language)))
    (cond ((eq language 'java)
	  ...
	  (t (concat "g++ -g -std=gnu++0x " topcoder-cpp-compile-flags
...
;; (defun write-to-topcoder-dir(message)
;;   (let* ((dir (getenv "EMACS_TOPCODER_DIR")))
;;     (write-string-to-file message (concat dir "\\message.txt"))
;;     ))
